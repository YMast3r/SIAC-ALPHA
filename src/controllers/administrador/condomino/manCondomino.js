...
function manipulaCondomino(req, res) {
    ...
    req.getConnection((err, conn) => {
        if (err) {
            ...
            req.session.errorMC = 'Error: Problema con la conexión a la base de datos!';
        ...
        conn.query(consulta, parametros, (err, userdata) => {
            ...
            if (userdata.length > 0) {
                    ...
                    req.getConnection((err, conn) => {
                        ...
                        conn.query(consulta2, parametros2, (err, rows) => {
                            if (err) {
                                ...
                                req.session.errorMC = 'Error: Al actualizar los datos!';
                                res.redirect('/ruta-de-error'); // Redirige a una ruta de error
                                return;
                            }
                            renderManCondomino(req, res); 
                            ...
                            req.getConnection((err, conn) => {
                                ...
                                conn.query(consulta2, parametros2, (err, rows) => {
                                    if (err) {
                                        ...
                                        req.session.errorMC = 'Error: Al ingresar los datos!';
                                        res.redirect('/ruta-de-error'); // Redirige a una ruta de error
                                        return;
                                    }
                                    renderManCondomino(req, res); 
                ...
                } // termina la condición si la contraseña se mantiene
            }
        });
    });
}
