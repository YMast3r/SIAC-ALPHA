...
function manAdm(req, res) {
    ...
    const error = req.session.errorMSA;
...
function ediAdm(req, res){
    ...
    let error = req.session.errorMSA;
    if (req.params.id){
...
function manipulaAdm(req, res){
    ...
    if (!passIgual){
        if (data.password !== data.confPassword) {
            req.session.errorMSA = 'Error: La contraseña y la confirmación no coinciden';
                    ...
                    req.getConnection((err, conn) => {
                        ...
                        conn.query(consulta2, parametros2, (err, rows) => {
                            if (err) {
                                ...
                                req.session.errorMSA = 'Error: Al ingresar los datos!';
                                manAdm(req, res);
                                return;
                            }
                            renderAdm(req, res);
